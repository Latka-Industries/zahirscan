...
- **Tabular / columnar**: Parquet; Arrow IPC / Feather (`.arrow`, `.feather`, `.ipc`); Avro; ORC; NumPy (`.npy`, `.npz`); HDF5 (`.h5`, `.hdf5`); NetCDF (`.nc`, `.cdf`); Matrix Market (`.mtx`); MATLAB (`.mat`); Zarr (`.zarr`); [Tetration][tetration] ([`.tet`][tetration])
- **Models**: ONNX (`.onnx`), GGUF (`.gguf`), TensorFlow Lite (`.tflite`), Safetensors (`.safetensors`)
...
| Tabular / columnar    | **Parquet, Arrow IPC/Feather, Avro, ORC**: schema names/types, row counts, `stats_rows_sampled`, and bounded column stats. **NumPy** (`.npy`/`.npz`), **HDF5**, **NetCDF**, **Matrix Market** (`.mtx`), **MATLAB** (`.mat`), **Zarr** (`.zarr`): shapes, dtypes, format-specific variable/column summaries. Shared tabular sample caps apply; **bytes-per-row** applies when row count is known before stats. **[Tetration][tetration]** ([`.tet`][tetration]): catalog, datasets, chunk index, query-derived column/tensor stats.                                             |
| Models                | **ONNX** (`.onnx`): producer/opset/op-type summary, I/O + initializer counts, typed graph I/O, node count. **GGUF** (`.gguf`): key/value metadata and tensor table summary. **TFLite** (`.tflite`): model/subgraph/operator summary with tensor descriptors. **Safetensors** (`.safetensors`): optional `__metadata__`, tensor summaries, dtype histograms, and `parse_ok`/`parse_error` status.                                                                                                                                                                               |
