...
- **Tabular / columnar**: Parquet; Arrow IPC / Feather (`.arrow`, `.feather`, `.ipc`); Avro; ORC; NumPy (`.npy`, `.npz`); HDF5 (`.h5`, `.hdf5`); NetCDF (`.nc`, `.cdf`); Matrix Market (`.mtx`); MATLAB (`.mat`)
...
| Tabular / columnar    | **Parquet, Arrow IPC/Feather, Avro, ORC**: schema names/types, row counts, `stats_rows_sampled`, and bounded column stats. **NumPy** (`.npy`/`.npz`), **HDF5**, **NetCDF**, **Matrix Market** (`.mtx`), **MATLAB** (`.mat`): shapes, dtypes, and format-specific variable/column summaries. Shared tabular sample caps apply; **bytes-per-row** applies when row count is known before stats.                                                                                                                                                                                  |
