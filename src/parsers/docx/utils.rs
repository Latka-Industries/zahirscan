...
//!
//! Contains reusable helpers for XML parsing, entity decoding, and ZIP archive handling.
...
/// Open a DOCX/XLSX file as a ZIP archive
///
/// Office files (DOCX, XLSX, etc.) are ZIP archives containing XML files.
/// This helper function opens the content as a ZIP archive with proper error handling.
///
/// # Arguments
/// * `content` - The raw bytes of the Office file
...
///
/// Performs basic XML entity replacement for common entities.
/// This is used as a fallback when the primary `unescape()` function fails.
...
///
/// Attempts to use quick-xml's `unescape()` function first, falling back
/// to manual entity replacement if that fails.
