...
#[inline]
fn read_u16_safe(data: &[u8], offset: usize, is_little_endian: bool) -> Option<u16> {
...
/// Returns None if offset is out of bounds
#[inline]
fn read_u32_safe(data: &[u8], offset: usize, is_little_endian: bool) -> Option<u32> {
...
pub fn extract_compression(tiff_data: &[u8]) -> Option<String> {
    if tiff_data.len() < 8 {
        return None;
    }
    ...
    let version = read_u16_safe(tiff_data, 2, is_little_endian)?;
...
pub fn extract_bit_depth(tiff_data: &[u8]) -> Option<u32> {
    ...
    for i in 0..max_entries {
        ...
        if tag == TIFF_TAG_PHOTOMETRIC_INTERPRETATION {
            ...
            let value = if entry_type == TIFF_TYPE_SHORT {
                ...
                match read_u16_safe(tiff_data, read_offset, is_little_endian) {
                    Some(v) => v as u32,
                    None => continue,
                }
            } else if entry_type == TIFF_TYPE_LONG {
                // LONG: read all 4 bytes
                match read_u32_safe(tiff_data, entry_offset + 8, is_little_endian) {
                    Some(v) => v,
                    None => continue,
                }
            } else {
                ...
                if is_little_endian && entry_offset + 10 <= tiff_data.len() {
                    // Try reading as SHORT from bytes 8-9
                    let value = match read_u16_safe(tiff_data, entry_offset + 8, is_little_endian) {
                        Some(v) => v as u32,
                        None => continue,
                    };
                    return match value {
